// Structure des départements français organisés par région
...
// Liste des départements français organisés par région
...
// Helper pour récupérer les régions uniques
export const getUniqueRegions = (): string[] => {
  const regions = frenchDepartments.map(dept => dept.region);
  return [...new Set(regions)].sort();
};

// Helper pour récupérer les départements d'une région
export const getDepartmentsByRegion = (region: string): Department[] => {
  return frenchDepartments
    .filter(dept => dept.region === region)
